...
hook.Add( "InitPostEntity", "essyrev_nick_over_head", function()
	...
	hook.Add( "player_connect_client", "essyrev_nick_over_head", function( data )
	...
	hook.Add( "PostDrawTranslucentRenderables", "PaintManual Test", function()
		...
		for player, frame in pairs(players) do
			...
			cam.Start3D2D(player:GetBonePosition( player.bone_head ) + Vector(0,0,25), Angle(0,angle[2] + 90,90), 0.05)
				frame[1]:PaintManual()
			cam.End3D2D()
		end
